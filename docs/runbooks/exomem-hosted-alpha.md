...
2. the immutable Exomem `0.68.1` cell image from commit
   `e487efa2fdfd8c7653b6e99605163a0200c6ce58`, pinned as
...
current `0.68.1` agent-and-gateway projection recipe is in
...
| `EXOMEM_CELL_RELEASE_VERSION`                                                    | Exact deployed Exomem release, pinned to `0.68.1` for this release unit -- the release the hosted deployment lock names. Readiness must echo it, and the gateway contract catalog must carry the exact v4 fixture pair for it or every command fails closed.                                                                                                          |
...
For the `0.68.1` release, both clients must discover profile
...
2. `preflight`. Read-only, spends nothing. All four gates must be green: no live
   ...
   `0.68.1`. Only the default candidate's locks sit at the generated root; every
...
1. Verify privately that the candidate is pending `hosted-alpha-agent-v4`
   release `0.68.1`, contains exactly 25 ordered commands, the selected client
...
The current release is Exomem `0.68.1` at
`e487efa2fdfd8c7653b6e99605163a0200c6ce58`: command surface
...
`6743cf711b08cf8b64a7db8a62ce06f4a9246e59cc54a76f23c102959fc10aa9`
and the image digest is
`sha256:9870b3f661969a70504fb4ccad60b6429c21c13732f754d0e8aef030e3277246`.
...
node scripts/generate-exomem-hosted-contract.mjs \
  ...
  --gateway-output src/lib/exomem-hosted/gateway-contract-0-68-1.ts \
  --gateway-json-output src/lib/exomem-hosted/__tests__/gateway-contract-0-68-1.json \
  --expected-commit e487efa2fdfd8c7653b6e99605163a0200c6ce58 \
  --source-release 0.68.1
```

The explicit `-0-34-0`, `-0-35-0`, `-0-39-2`, `-0-49-0`, `-0-50-0`, `-0-54-1`,
`-0-57-2`, `-0-63-1`, `-0-66-0`, and `-0-68-0` fixtures are retained historical units with their
original profiles; do not overwrite or regenerate them while refreshing the
current release. `-0-68-0` is the newest and the one most at risk: the previous
...
Import the current `0.68.1` catalog unit through the current control only:
...
`import-agent` imports the bare current `0.68.1` fixture. Do not send `0.68.1`
...
`0.57.2/v1` to `0.68.1/v4`; rerun it only when changing the outer provisioner
...
`0.68.1` reviewer rollforward carries `hosted-alpha-agent-v4`, while historical
...
current `0.68.1` catalog unit as a fresh pending candidate, use
...
`0.68.1`. Retiring a release ships its retained fixture so live cells keep
routing, but does not widen this control -- `0.57.2`, `0.63.1`, `0.66.0`, and `0.68.0`
