...
current `0.73.1` agent-and-gateway projection recipe is in
...
| `EXOMEM_CELL_RELEASE_VERSION`                                                    | Exact deployed Exomem release, pinned to `0.73.1` for this release unit -- the release the hosted deployment lock names. Readiness must echo it, and the gateway contract catalog must carry the exact v4 fixture pair for it or every command fails closed.                                                                                                          |
...
For the `0.73.1` release, both clients must discover profile
...
2. `preflight`. Read-only, spends nothing. All four gates must be green: no live
   ...
   `0.73.1`. Only the default candidate's locks sit at the generated root; every
...
The current release is Exomem `0.73.1` at
`956dcdbef56a62a697756c0369f5580a91ced35a`: command surface
...
compatibility `636d271faaa57d38730a5638abb9f12797cb49189e99be9762632f03ae49117c`,
...
node scripts/generate-exomem-hosted-contract.mjs \
  ...
  --gateway-output src/lib/exomem-hosted/gateway-contract-0-73-1.ts \
  --gateway-json-output src/lib/exomem-hosted/__tests__/gateway-contract-0-73-1.json \
  --expected-commit 956dcdbef56a62a697756c0369f5580a91ced35a \
  --source-release 0.73.1
```

The explicit `-0-34-0`, `-0-35-0`, `-0-39-2`, `-0-49-0`, `-0-50-0`, `-0-54-1`,
`-0-57-2`, `-0-63-1`, `-0-66-0`, `-0-68-0`, `-0-68-1`, `-0-68-3`, and `-0-72-1` fixtures are retained historical units with their
original profiles; do not overwrite or regenerate them while refreshing the
current release. `-0-72-1` is the newest and the one most at risk: the previous
...
Import the current `0.73.1` catalog unit through the current control only:
...
`import-agent` imports the bare current `0.73.1` fixture. Do not send `0.73.1`
...
current `0.73.1` catalog unit as a fresh pending candidate, use
...
`0.73.1`. Retiring a release ships its retained fixture so live cells keep
routing, but does not widen this control -- `0.57.2`, `0.63.1`, `0.66.0`, `0.68.0`, `0.68.1`, `0.68.3`, and `0.72.1`
