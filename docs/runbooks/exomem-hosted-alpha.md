...
2. the immutable Exomem `0.68.0` cell image from commit
   `76571f2c9f600395344a2a62efe6aca36d32b42d`, pinned as
...
current `0.68.0` agent-and-gateway projection recipe is in
...
| `EXOMEM_CELL_RELEASE_VERSION`                                                    | Exact deployed Exomem release, pinned to `0.68.0` for this release unit -- the release the hosted deployment lock names. Readiness must echo it, and the gateway contract catalog must carry the exact v4 fixture pair for it or every command fails closed.                                                                                                          |
...
For the `0.68.0` release, both clients must discover profile
...
2. `preflight`. Read-only, spends nothing. All four gates must be green: no live
   ...
   `0.68.0`. Only the default candidate's locks sit at the generated root; every
...
1. Verify privately that the candidate is pending `hosted-alpha-agent-v4`
   release `0.68.0`, contains exactly 25 ordered commands, the selected client
...
The current release is Exomem `0.68.0` at
`76571f2c9f600395344a2a62efe6aca36d32b42d`: command surface
`4b4b71280fec7915042483207b1ab0e15e916148ac1b88ef965e03671de80968`, schema
`124fb718c6d2b6caee93edd7281fbc6cd7ca991e4a39bcc90df00bf0811208fd`,
compatibility `62356a1220b823e9ae91e1fab18a8da5711481b6cc907dbcae033e254a3585dc`,
...
and private gateway `4e19849239188017b727a7ec97fe6e8505a01d216907957755676f3f588b8cd6`.
The immutable runtime candidate is SHA-256
`e6a98f21bc4910f320b959d510989dc96c5c0746c0f7957aff3f5748eac85784`
and the image digest is
`sha256:78762e5676a57fff444d1360a968ba9d34d9cb5e6032f80542b813645ce765b0`.
...
node scripts/generate-exomem-hosted-contract.mjs \
  ...
  --gateway-output src/lib/exomem-hosted/gateway-contract-0-68-0.ts \
  --gateway-json-output src/lib/exomem-hosted/__tests__/gateway-contract-0-68-0.json \
  --expected-commit 76571f2c9f600395344a2a62efe6aca36d32b42d \
  --source-release 0.68.0
```

The explicit `-0-34-0`, `-0-35-0`, `-0-39-2`, `-0-49-0`, `-0-50-0`, `-0-54-1`,
`-0-57-2`, `-0-63-1`, and `-0-66-0` fixtures are retained historical units with their
original profiles; do not overwrite or regenerate them while refreshing the
current release. `-0-66-0` is the newest and the one most at risk: the previous
...
Import the current `0.68.0` catalog unit through the current control only:
...
`import-agent` imports the bare current `0.68.0` fixture. Do not send `0.68.0`
...
`0.57.2/v1` to `0.68.0/v4`; rerun it only when changing the outer provisioner
...
`0.68.0` reviewer rollforward carries `hosted-alpha-agent-v4`, while historical
...
current `0.68.0` catalog unit as a fresh pending candidate, use
...
`0.68.0`. Retiring a release ships its retained fixture so live cells keep
routing, but does not widen this control -- `0.57.2`, `0.63.1`, and `0.66.0`
