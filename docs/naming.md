...
The claim email and the claim page tell a buyer to open **Hosted Backup** in the
desktop app, because that is the label the app shows today (`endstate-gui`
renders "Hosted Backup" in its navigation, chip, and empty states). Sending
someone to look for a screen that does not exist under the name we printed
would be worse than an inconsistent noun, so the instruction names the real
label and qualifies it: "open Hosted Backup (the app still uses the old name)".

**When `endstate-gui` ships the rename, update these two strings and drop the
qualifier:**
