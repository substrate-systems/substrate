...
> Skill-triggered contract loading produced a substantial quality improvement over bulk loading via `AGENTS.md` — from a **7/10 baseline** to **9–9.5/10** on the same task.
...
**Retrieval over available context.** Content sits in `AGENTS.md`. The model is told it's there. To use it, the model has to:
...
**Instruction-following.** A skill triggers on the task pattern. The content arrives in the context already framed as instructions for the current task. The model executes against them. *There is no engagement decision.* The content is already in the role the model is trained to follow.
...
A skill cuts that step out. The trigger does the binding. The model receives the relevant rules as its current operating instructions.

## What this looks like in practice

**Advisory framing** — rules sitting in `AGENTS.md`, available to the model:
...
The fix is the same fix: **bind retrieval to a skill or contract** that frames the loaded content as the operating context for the response, not as ambient reference. The retrieval mechanism itself doesn't change. The cognitive operation the model performs on the retrieved content does.
...
The skill is the binding layer. Docs don't need to be rewritten. The trigger does the work.
