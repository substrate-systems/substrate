...
The reconciler no longer chooses a cell release from the process-wide `EXOMEM_CELL_RELEASE_VERSION` at execution time. Every provision or restore operation snapshots one server-selected target candidate, assignment generation when present, source release, protocol, agent locks, and full private-gateway contract digest before its first side effect. For an assigned rollout this target comes from the preparing assignment; for ordinary work it comes from the current live cohort. The snapshot is immutable across leases, waits, retries, restarts, and a concurrent cohort promotion. A canary roll is an explicit quiesce/export/restore/rebind sequence: create the preparing assignment, enqueue the restore with the assignment snapshot, keep the old live route authoritative while the replacement is unready, require the provisioner health response to report the exact verified gateway digest plus agent release/protocol/fingerprint/schema/compatibility locks, then atomically bind and activate. Missing or mismatched attestation leaves the old binding intact or the tenant fail-closed; it never falls through to another release, fixture, or cell.
