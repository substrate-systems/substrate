...
`POST /api/backups/:id/versions` treats _creating_ a version as _completing_
...
- The phantom counts against the 1 GiB quota and against
  ...
  stored.
- GC cannot reconcile it: Pass C only HEADs the _manifest_, only after 48
...
- Keep every new version pending, regardless of the caller version. Schema
  2.1+ clients publish with an explicit commit; older callers are reconciled
  only after the server has verified every expected R2 object and its length.
...
No field is re-typed, so this is additive per contract §13. Migration 0040
retains every existing version and account but marks historical generations
`legacy_unverified`. Release A preserves pre-cutoff history through a
DB-governed, non-strict bridge while a bounded R2 verifier checks its complete
object sets. A separately confirmed strict cutover refuses until that history
has drained; rows created after the cutoff are never bridge-visible.

## Impact

- Affected specs: `hosted-backup-operations`
- Affected code:
  - `migrations/0040_backup_version_commit.sql`
