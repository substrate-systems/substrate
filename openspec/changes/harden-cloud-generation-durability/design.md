...
Migration `0038_backup_version_commit.sql` is additive and online-safe:
`committed_at` is nullable, `requires_commit` defaults to `false`.

The backfill (`UPDATE backup_versions SET committed_at = created_at WHERE
committed_at IS NULL`) is the load-bearing compatibility step: it runs before
any code reads the new predicate, and combined with the `false` default it
means every pre-existing version passes visibility on both clauses. There is
no window in which a deployed predicate can hide existing history.

Rollback: revert the application code. The two columns can be left in place
harmlessly — with `requires_commit = false` everywhere, the predicate is a
tautology and behaviour is identical to 2.0.
