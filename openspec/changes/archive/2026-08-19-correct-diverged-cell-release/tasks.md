...
- [x] 5.2 Re-issue the deletion for tenant `1809ce5c` and confirm it reaches `succeeded`
  - **This change cannot do it, and the premise that it could was wrong.** An operation copies its target at creation and the reconciler builds the request from the operation, not the cell, so the stranded delete still presents 0.50.0. A fresh delete cannot be minted either, because `consumeDeletionConfirmationAtomic` refuses a `deletion_pending` tenant. Discharged instead by `supersede-stranded-cell-delete`, which restores the tenant to the target-free delete shape admission cannot reject; that change applied it in production on 2026-08-19T09:34Z and the replacement reached `succeeded` at checkpoint `destroyed`.
- [x] 5.3 Confirm the routable set is empty and capacity is released before provisioning the alpha tenants
  - Load-bearing rather than tidiness: promotion health-probes every routable cell and requires every probe to succeed, and this cell yielded no probe at all (its origin provision sat at the old fence and the old candidate). Measured `probeable_ops = 0` on 2026-08-19. After the supersession the routable set is empty, all four runtime slots are free, and the bootstrap preflight is green on every gate.
