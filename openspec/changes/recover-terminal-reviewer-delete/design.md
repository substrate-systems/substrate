...
The contracts admin route gains `preflight-recover-terminal-reviewer-delete` and `recover-terminal-reviewer-delete`. Each accepts only the exact delete operation UUID and expected fence. The mutation retains `operation_type=delete`, the target-free identity, fence, idempotency key, and checkpoint `destroyed`; that persisted checkpoint is the authoritative provider-destruction proof. It clears terminal/error/lease metadata, resets attempts, and schedules the same row as `waiting` once. Because the reconciler's `destroyed` branch calls only `markCellState`, no provider request occurs.
