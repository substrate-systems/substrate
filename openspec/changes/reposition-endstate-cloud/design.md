...
*Alternative considered:* rename everything and migrate. Rejected — the
...
*Alternative considered:* calling the Paddle API to create the products. Ruled
...
### "Kept current automatically" is qualified in the feature list

The approved outcome line is used as written, and the tier's feature list says
"Optional automatic backup after each capture" — which is what `endstate-gui`
...
### The in-app fallback instruction keeps the old label, qualified

`endstate-gui` still labels its section "Hosted Backup". The claim email and
claim page tell a buyer to open it, so those two strings name the real label and
say why: "open Hosted Backup (the app still uses the old name)".

Renaming them would send a paying customer to look for a screen that does not
exist under the name printed in their email. An inconsistent noun is the cheaper
error. `docs/naming.md` names both strings and the test that pins them, so the
cross-repo follow-up is a mechanical edit rather than a rediscovery.

## Risks / Trade-offs

- **Two names in circulation during the transition** (site says Endstate Cloud,
  app says Hosted Backup) → the Terms parenthetical, the qualified fallback
  instruction, and a documented follow-up in `docs/naming.md`.
...
- When `endstate-gui` renames its section, the two qualified strings above and
  the `hostedBackup` capabilities key should be revisited together. The key is a
  wire contract and may still need to stay.
