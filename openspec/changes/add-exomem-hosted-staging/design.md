...
The permanent branch is `staging`; `staging.substratesystems.io` is assigned only to that branch. Branch-scoped Preview variables set the public origin, database, Paddle Sandbox resources, scheduler/admin secrets, and existing provisioner credentials. Production variables remain untouched. The authoritative DNS provider carries the exact Vercel-requested DNS-only record. Because Hobby Standard Protection intercepts Preview requests before the application and cannot exempt one domain, Vercel Authentication is disabled for this project while permanent staging is active; application auth remains authoritative. This intentionally makes all project Preview URLs public at the Vercel edge so Paddle webhooks and ChatGPT MCP/OAuth can reach staging.
...
Staging reuses the existing non-production `exomem_restore_verification` database and owns a fresh `exomem_hosted_staging` schema selected first in the connection `options` search path. The unpooled Neon hostname is required because its pooler rejects `search_path` startup options. Neon HTTP does not apply those session options, so a URL that declares `search_path` routes ordinary application SQL through the same session-backed PostgreSQL pool used for transactions. The exact path is `exomem_hosted_staging,public`: staging tables, sessions, and migrations resolve into the first schema, while existing extension types remain visible from `public` and the earlier verification fixtures there stay untouched. The Vercel migration hook may run for a Preview only when all three facts agree: `VERCEL_ENV=preview`, `VERCEL_GIT_COMMIT_REF=staging`, and `EXOMEM_HOSTED_STAGING=true`. It verifies that `DATABASE_URL` names both the configured staging database and exact staging search path, then verifies that PostgreSQL's effective `current_schema()` is that schema before touching the migration ledger. The live check matters because PostgreSQL silently skips nonexistent schemas in `search_path` and would otherwise fall through to `public`.
