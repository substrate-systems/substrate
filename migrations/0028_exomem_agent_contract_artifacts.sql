...
CREATE TABLE exomem_agent_contract_candidates (
  ...
  claude_package_lock jsonb NOT NULL CHECK (jsonb_typeof(claude_package_lock) = 'object'),
  claude_archive_lock jsonb NOT NULL CHECK (jsonb_typeof(claude_archive_lock) = 'object'),
  openai_package_lock jsonb CHECK (openai_package_lock IS NULL OR jsonb_typeof(openai_package_lock) = 'object'),
  openai_archive_lock jsonb CHECK (openai_archive_lock IS NULL OR jsonb_typeof(openai_archive_lock) = 'object'),
...
CREATE TABLE exomem_client_artifacts (
  ...
  paired_run_hmac_sha256 text NOT NULL CHECK (char_length(paired_run_hmac_sha256) = 64),
  exomem_identity_hmac_sha256 text NOT NULL CHECK (char_length(exomem_identity_hmac_sha256) = 64),
  tenant_hmac_sha256 text NOT NULL CHECK (char_length(tenant_hmac_sha256) = 64),
