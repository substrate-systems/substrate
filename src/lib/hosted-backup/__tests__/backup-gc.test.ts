...
type GcState = {
  ...
  cancelledExpiry: { graceExpired: number; downgraded: number; prefixesEnqueued: number };
...
function setup(overrides: Partial<GcState> = {}) {
  state = {
    ...
    cancelledExpiry: { graceExpired: 0, downgraded: 0, prefixesEnqueued: 0 },
  ...
  mock.module("../db", {
    namedExports: {
      ...
      listChunksForVersion: async (versionId: string) => state.chunksByVersion[versionId] ?? [],
      ...
      findLegacyUnverifiedVersions: async () => [],
      publishVerifiedLegacyVersion: async () => false,
      markLegacyVerificationFailed: async () => undefined,
...
describe("backup-gc — Pass F (stale uncommitted versions)", () => {
  it("deletes the R2 objects first, then hard-deletes the version row", async () => {
    setup({
      staleUncommittedVersions: [{ id: "v-uncommitted", manifest_object_key: "u/m-unc" }],
      chunksByVersion: {
...
describe("backup-gc — Pass G (post-cancellation purge)", () => {
  it("reports downgraded subscribers and the prefixes enqueued for purge", async () => {
    setup({ cancelledExpiry: { graceExpired: 1, downgraded: 3, prefixesEnqueued: 2 } });
    const res = await runGc();
    const body = await res.json();
    assert.equal(body.cancelledSubscriptionsExpired, 3);
    assert.equal(body.graceSubscriptionsCancelled, 1);
