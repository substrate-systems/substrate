...
const state = {
  ...
  createArgs: null as { requiresCommit?: boolean; operationId?: string } | null,
  createImpl: async (p: { requiresCommit?: boolean }) => ({
    versionId: "v-created",
...
before(() => {
  ...
  mock.module("@/lib/hosted-backup/storage", {
    namedExports: {
      ...
      createVersionWithUploads: async (p: { requiresCommit?: boolean; operationId?: string }) => {
        state.createArgs = p;
...
beforeEach(() => {
  ...
  state.createImpl = async (p: { requiresCommit?: boolean }) => ({
    versionId: "v-created",
    uploadUrls: [{ chunkIndex: -1, presignedUrl: "https://r2/x", expiresAt: "later" }],
    requiresCommit: p.requiresCommit === true,
  });
...
async function callCommit(backupId = "b-1", versionId = "v-1") {
  const { POST } =
    await import("../../../app/api/backups/[backupId]/versions/[versionId]/commit/route");
  const req = new Request(`http://localhost/api/backups/${backupId}/versions/${versionId}/commit`, {
    method: "POST",
  });
...
describe("POST /api/backups/:id/versions — schema negotiation", () => {
  ...
  it("a 2.0 client remains pending but receives a 2.0 response header it can parse", async () => {
    const { res, body } = await callCreateVersion({ "x-endstate-api-version": "2.0" });
    assert.equal(state.createArgs?.requiresCommit, true);
    assert.equal(body.requiresCommit, true);
    assert.equal(res.headers.get("X-Endstate-API-Version"), "2.0");
  });

  it("a client that sends no version header remains pending for server reconciliation", async () => {
    const { res, body } = await callCreateVersion({});
    assert.equal(state.createArgs?.requiresCommit, true);
    assert.equal(body.requiresCommit, true);
    assert.equal(res.headers.get("X-Endstate-API-Version"), "2.0");
  });

  it("uses X-Endstate-Operation-ID as the stable idempotency key on replay", async () => {
    const headers = { "x-endstate-operation-id": "engine-push-0001" };
    const first = await callCreateVersion(headers);
    assert.equal(first.res.status, 200);
    assert.equal(state.createArgs?.operationId, "engine-push-0001");

    const replay = await callCreateVersion(headers);
    assert.equal(replay.res.status, 200);
    assert.equal(state.createArgs?.operationId, "engine-push-0001");
  });

  it("passes a terminal committed replay through without inventing upload URLs", async () => {
    state.createImpl = async () => ({
      versionId: "v-committed",
      uploadUrls: [],
      requiresCommit: true,
      alreadyCommitted: true,
    });
    ...
    const { res, body } = await callCreateVersion({
      "x-endstate-operation-id": "engine-push-0001",
    });
    assert.equal(res.status, 200);
    assert.equal(body.versionId, "v-committed");
    assert.equal(body.alreadyCommitted, true);
    assert.deepEqual(body.uploadUrls, []);
  });

  it("rejects a body operationId that disagrees with the engine header", async () => {
    const { res, body } = await callCreateVersion(
      { "x-endstate-operation-id": "engine-push-0001" },
      { operationId: "different-body-id" }
    );
    assert.equal(res.status, 400);
    assert.equal((body.error as { code: string }).code, "BAD_REQUEST");
    assert.equal(state.createArgs, null);
  });

  it("rejects an unsupported request schema major before creating a version", async () => {
    const { res, body } = await callCreateVersion({ "x-endstate-api-version": "3.0" });
    assert.equal(res.status, 400);
    assert.equal((body.error as { code: string }).code, "BAD_REQUEST");
    assert.equal(state.createArgs, null);
  });
