...
describe("Python provisioner v2 interoperability corpus", () => {
  it("serializes every exact v2 body with its matching header and parses exact final responses", async () => {
    ...
    for (const [action, testCase] of Object.entries(v2Fixture.actions)) {
      ...
      if (action !== "health" && finalBody && typeof finalBody === "object") {
        const paddedFinalBody = { ...(finalBody as JsonRecord), forbiddenPadding: true };
        const paddedAdapter = new HttpCellProvisioner(
          {
            endpoint: new URL("https://provisioner.internal.example/v1"),
            credential: new SensitiveSecret("fixture-provisioner-bearer-000000000"),
            timeoutMs: 500,
          },
          async () => response(testCase.final.status, paddedFinalBody)
        );
        const paddedInvoke = async (): Promise<unknown> => {
          switch (action) {
          case "provision":
            return paddedAdapter.provision({ ...base, provisionMode: "serve" } as ProvisionCellRequest);
          case "rotate-credential":
            return paddedAdapter.rotateCredential({
              ...base,
              providerRef: String(body.providerRef),
              phase: body.phase as "stage" | "finalize",
              credentialVersion: Number(body.credentialVersion),
          ...
          case "export":
            return paddedAdapter.export({
              ...base,
              providerRef: String(body.providerRef),
              expiresAt: new Date(String(body.expiresAt)),
            } as never);
          case "export-delete":
            return paddedAdapter.deleteExport({
              context: base.context,
              tenantId: base.tenantId,
              exportRef: new SensitiveSecret(String(body.exportRef)),
              provisionerWireProtocol: base.provisionerWireProtocol,
          ...
          case "export-download":
            return paddedAdapter.createExportDownload({
              context: base.context,
              tenantId: base.tenantId,
              exportRef: new SensitiveSecret(String(body.exportRef)),
              provisionerWireProtocol: base.provisionerWireProtocol,
          ...
          case "discard":
            return paddedAdapter.discard({ ...base, providerRef: String(body.providerRef) } as never);
          case "destroy":
            return paddedAdapter.destroy({
              context: base.context,
              tenantId: base.tenantId,
              provisionerWireProtocol: base.provisionerWireProtocol,
            } as never);
          default:
            throw new Error(`unsupported padded v2 action: ${action}`);
          }
        };
        await assert.rejects(
          paddedInvoke(),
        ...
        const paddedAdapter = new HttpCellProvisioner(
          ...
          async () => response(200, { forbiddenPadding: true })
        );
        const paddedInvoke = async (): Promise<unknown> => {
          switch (action) {
          case "quiesce":
          ...
          case "stop":
          case "seal":
            return paddedAdapter[action]({ ...base, providerRef: String(body.providerRef) } as never);
          case "export-release":
            return paddedAdapter.releaseExport({
              ...base,
              providerRef: String(body.providerRef),
              releaseRef: new SensitiveSecret(String(body.releaseRef)),
            } as never);
          case "restore":
            return paddedAdapter.restore({
              ...base,
              providerRef: String(body.providerRef),
              restoreRef: new SensitiveSecret(String(body.restoreRef)),
              sourceCellId: String(body.sourceCellId),
          ...
          default:
            throw new Error(`unsupported padded v2 no-content action: ${action}`);
          }
        };
        await assert.rejects(
          paddedInvoke(),
  ...
  it("rejects a padded v2 worker policy", async () => {
    const health = materialize(v2Fixture.actions.health.final.body) as JsonRecord;
    const adapter = new HttpCellProvisioner(
      {
        endpoint: new URL("https://provisioner.internal.example/v1"),
        credential: new SensitiveSecret("fixture-provisioner-bearer-000000000"),
      ...
      async () =>
        response(200, {
          ...health,
          workerPolicy: { ...(health.workerPolicy as JsonRecord), forbiddenPadding: true },
        })
    );
    await assert.rejects(
      adapter.health({ ...v2ProvisionRequest(), providerRef: "hcloud.volume.fixture-alpha" }),
