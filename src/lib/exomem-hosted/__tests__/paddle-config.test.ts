...
describe("Exomem Paddle configuration", () => {
  it("binds a sandbox catalog and credentials to the shared sandbox client", () => {
    ...
    assert.equal(config.checkoutUrl, "https://substratesystems.io/exomem/home");
    assert.equal(config.clientEnvironment, "sandbox");
  ...
  it("keeps complimentary alpha usable when no paid price is configured", () => {
    ...
    assert.doesNotThrow(() => assertExomemPaddlePurpose(config, "transaction"));
    assert.throws(
  ...
  it("loads exact transaction access independently of unrelated checkout configuration", () => {
    const env: Record<string, string> = {
      ...sandboxEnv(),
      NEXT_PUBLIC_PADDLE_CLIENT_TOKEN: "live_client_token",
      NEXT_PUBLIC_PADDLE_ENVIRONMENT: "production",
      ...
      EXOMEM_PADDLE_PRICE_ID: "pri_historical",
    };
    delete env.EXOMEM_PADDLE_PRODUCT_ID;

    const config = loadExomemPaddleTransactionConfig(env);

    assert.equal(config.environment, "sandbox");
    assert.equal(config.apiKey, "pdl_sdbx_apikey_example");
    assert.equal(config.productId, null);
  ...
  it("fails closed when the Paddle environment is absent or ambiguous", () => {
    ...
    const unknownRuntime = { ...sandboxEnv(), NODE_ENV: "preview" };

    for (const env of [missing, liveAlias, unknownRuntime]) {
      assert.throws(
  ...
  it("rejects detectably mixed sandbox/live credentials and catalog bindings", () => {
    ...
    for (const env of [mixedKey, mixedCatalog, mixedBrowser]) {
      assert.throws(
        () => loadExomemPaddleConfig(env),
        (error: unknown) =>
          error instanceof ExomemPaddleConfigurationError &&
  ...
  it("fails paid checkout closed without a configured public checkout origin", () => {
    const env = sandboxEnv();
    delete env.EXOMEM_PUBLIC_BASE_URL;
    const config = loadExomemPaddleConfig(env);
  ...
  it("fails paid checkout closed without an explicit browser Paddle environment", () => {
    const env = sandboxEnv();
    delete env.NEXT_PUBLIC_PADDLE_ENVIRONMENT;
    const config = loadExomemPaddleConfig(env);
