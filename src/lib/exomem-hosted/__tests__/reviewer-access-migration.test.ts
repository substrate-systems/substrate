...
test("reviewer access persists provider-scoped secret digests and session attribution", () => {
  ...
  assert.match(sql, /CREATE UNIQUE INDEX exomem_marketplace_reviewer_credentials_active_provider_idx[\s\S]*WHERE revoked_at IS NULL/i);
  assert.match(sql, /ADD COLUMN reviewer_credential_id uuid REFERENCES exomem_marketplace_reviewer_credentials\(id\) ON DELETE SET NULL/i);
  assert.match(sql, /ALTER TABLE exomem_oauth_authorization_transactions[\s\S]*ADD COLUMN reviewer_credential_id uuid REFERENCES exomem_marketplace_reviewer_credentials\(id\) ON DELETE SET NULL/i);
  assert.match(sql, /ALTER TABLE exomem_oauth_grants[\s\S]*ADD COLUMN reviewer_credential_id uuid REFERENCES exomem_marketplace_reviewer_credentials\(id\) ON DELETE SET NULL/i);
  ...
  assert.match(sql, /CREATE INDEX exomem_sessions_reviewer_credential_active_idx[\s\S]*WHERE reviewer_credential_id IS NOT NULL AND revoked_at IS NULL/i);
