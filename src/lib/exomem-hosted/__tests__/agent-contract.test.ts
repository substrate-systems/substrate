...
describe("Exomem Hosted agent contracts", () => {
  ...
  it("ships retained 0.57.2/v1 through 0.73.1/v4 beside the exact current 0.74.0/v4 fixture", () => {
    ...
    assert.equal(
      ...
      "bd95fc9826069ec66f142c821abfda4b2f1d0912"
    );
    assert.equal(exomemHostedContractFixture.sourceRelease, "0.74.0");
  ...
  // of Claude locks whose first entry is the current release. Adopting 0.74.0
  // rotates that entry off 0.73.1, so 0.73.1 must be added explicitly or the
  ...
  it("trusts the fixture source release independently of descriptor source_release", async () => {
    ...
    try {
      ...
      assert.equal(fixture.sourceRelease, "0.74.0");
