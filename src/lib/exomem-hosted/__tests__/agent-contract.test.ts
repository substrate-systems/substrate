...
describe("Exomem Hosted agent contracts", () => {
  ...
  it("ships retained 0.57.2/v1, 0.63.1/v4, 0.66.0/v4, 0.68.0/v4, 0.68.1/v4, 0.68.3/v4, and 0.72.1/v4 beside the exact current 0.73.1/v4 fixture", () => {
    for (const retained of ["0-57-2", "0-63-1", "0-66-0", "0-68-0", "0-68-1", "0-68-3", "0-72-1"]) {
      assert.equal(
        existsSync(
    ...
    assert.equal(
      ...
      "956dcdbef56a62a697756c0369f5580a91ced35a"
    );
    assert.equal(exomemHostedContractFixture.sourceRelease, "0.73.1");
  ...
  it("trusts the fixture source release independently of descriptor source_release", async () => {
    ...
    try {
      ...
      assert.equal(fixture.sourceRelease, "0.73.1");
