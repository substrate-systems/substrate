...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.66.0 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0660.release), true);
    assert.equal(values.includes(exomemContractFixture0660.digest), true);
  });
