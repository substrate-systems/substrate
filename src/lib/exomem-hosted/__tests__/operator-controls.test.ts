...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.63.1 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0631.release), true);
    assert.equal(values.includes(exomemContractFixture0631.digest), true);
  });
