...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.68.0 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0680.release), true);
    assert.equal(values.includes(exomemContractFixture0680.digest), true);
  });
