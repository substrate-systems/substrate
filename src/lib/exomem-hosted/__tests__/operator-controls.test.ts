...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.72.1 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0721.release), true);
    assert.equal(values.includes(exomemContractFixture0721.digest), true);
  });
