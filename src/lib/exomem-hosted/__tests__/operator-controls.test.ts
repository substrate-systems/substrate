...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.74.0 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0740.release), true);
    assert.equal(values.includes(exomemContractFixture0740.digest), true);
  });
