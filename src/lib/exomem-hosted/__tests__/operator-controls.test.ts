...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.77.0 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0770.release), true);
    assert.equal(values.includes(exomemContractFixture0770.digest), true);
  });
