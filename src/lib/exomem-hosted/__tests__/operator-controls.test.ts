...
describe("hosted operator controls", () => {
  it("stages a virgin bootstrap authority against the exact 0.73.1 gateway contract", async () => {
    ...
    assert.equal(values.includes(exomemContractFixture0731.release), true);
    assert.equal(values.includes(exomemContractFixture0731.digest), true);
  });
