...
import { exomemHostedContractFixture as agentFixture0731 } from "../agent-contract-fixture";
...
describe("registry-derived Exomem gateway", () => {
  ...
  it("keeps live and candidate agent contracts paired with their own full fixtures", async () => {
    for (const [releaseVersion, hosted] of [
      ...
      ["0.73.1", CURRENT_HOSTED_CONTRACT],
    ] as const) {
      const row = target({
        userId: USER_A,
        tenantId: TENANT_A,
  ...
  for (const [label, hosted, full] of [
    ["current 0.73.1", CURRENT_HOSTED_CONTRACT, FULL_CONTRACT_0731],
