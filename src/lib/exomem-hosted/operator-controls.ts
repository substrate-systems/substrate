    ...
    const { rows } = await tx`
      ...
      WITH invite AS (
        ...
        JOIN exomem_agent_contract_candidates AS candidate
         ...
         AND candidate.profile_id = ${EXOMEM_HOSTED_PROFILE}
         AND candidate.source_release = ${exomemContractFixture0731.release}
         AND candidate.protocol_version = ${exomemContractFixture0731.protocol}
         AND candidate.state = 'pending'
        ...
        SELECT 'active', invite.id, stage.candidate_id, stage.profile_id, stage.contract_sha256,
               stage.source_release, stage.protocol_version, ${exomemContractFixture0731.digest},
