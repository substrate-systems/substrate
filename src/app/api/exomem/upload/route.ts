...
import { createDirectTransferTicket, type UploadMetadataV2 } from "@/lib/exomem-hosted/transfers";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const ticket = await createDirectTransferTicket({
      session,
      request: { operation: "upload", metadata: body.metadata as UploadMetadataV2 },
    });
